...
volatile int WebCmd = 0;                      //Command from the cloud website: 0 means no sampling, 1 means start sampling, 2 means stop sampling!
...
volatile int DeviceStatus = 0;                //MAD-AS device status: 0 means not sampling, 1 means is sampling.
...
////////////////////////////////////Main Program////////////////////////////////////
void loop() {
  while (WebCmd == 0){ //Repeatedly check command from the cloud every 30 seconds, until the command is YES.
    Serial.println("Controller starts checking command from the cloud...");
    CheckWebCmd();
    if (WebCmd == 0 || WebCmd == 2){
      Serial.println("No start command is given from the cloud. Next check is due in 30 seconds.");
      Sleepy(30);
    }else{
      ...
      WakeUp();
      if (DeviceWakeUp == 1){
        Serial.println("The nearby MAD-AS device has received the command and its response is now captured.");
        HC12Sleep();
      }
    }
  }
...
void WakeUp(){
  while (DeviceWakeUp == 0){
    //Send command to the nearby device for 1 second
    Serial.println("Sending command over radio...");
    long StartTime = millis();
    while(millis() - StartTime < 1000){
        HC12.write(WebCmd);
    }
    //Listen from the nearby device for 1 second
    Serial.println("Listening for response over radio...");
    StartTime = millis();
    while( millis() - StartTime < 1000 ){
      if(HC12.available()){
        DeviceWakeUp = HC12.read();
      }
    }
    if(DeviceWakeUp == 1){
      DeviceStatus = 1;
